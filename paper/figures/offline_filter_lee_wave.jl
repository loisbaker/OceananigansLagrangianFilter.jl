...
simulation = Simulation(model, Δt=2seconds, stop_time=15days)

# Set an adaptive timestep
conjure_time_step_wizard!(simulation, IterationInterval(20), cfl=0.2, max_Δt=20seconds)
...
filter_config = OfflineFilterConfig(original_data_filename="data/lee_wave.jld2", # Where the original simulation output is
                                    output_filename = "data/lee_wave_offline_filtered.jld2", # Where to save the filtered output
                                    var_names_to_filter = ("b",), # Which variables to filter
                                    velocity_names = ("u","w"), # Velocities to use for remapping
                                    architecture = GPU(), # CPU() or GPU(), if GPU() make sure you have CUDA.jl installed and imported
